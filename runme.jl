...
@time P_cbf, az_list = cbf(Rx, sensors2, freq_interest, c0);
println("\nFor DBF (Coordinate-Free) Beamformer: ")
order = 3;
@time P_dbf, az_list = dbf_coFree(Rx, sensors2, order, freq_interest, c0);
println("\nFor MUSIC Beamformer: ");
@time P_music, az_list = music(Rx, sensors2, 1, freq_interest, c0); # MUSIC
println("\nFor MVDR/MPDR Beamformer: ");
@time P_mvdr, az_list = mvdr(Rx, sensors2, freq_interest, c0); # Technically MPDR
println("\n")
...
# @time P_dbf, az_list = dbf(new_S, sensors2, order)
