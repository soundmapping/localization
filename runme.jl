...
@time P, az_list = cbf(Rx, sensor);
println("\nFor MUSIC Beamformer: ");
@time P_music, az_list = music(Rx, sensor, 1); # MUSIC
println("\nFor MVDR/MPDR Beamformer: ");
@time P_mvdr, az_list = mvdr(Rx, sensor); # Technically MPDR
println("\n")
...
@time P_dbf, az_list = dbf(new_S, sensor, order)
